...
def get_aws_clients():
    ...
    dynamodb = boto3.resource("dynamodb", endpoint_url=FLOCI_ENDPOINT, region_name=REGION, aws_access_key_id=access_key, aws_secret_access_key=secret_key)
    s3 = boto3.client("s3", endpoint_url=FLOCI_ENDPOINT, region_name=REGION, aws_access_key_id=access_key, aws_secret_access_key=secret_key)
    ...
    try:
        ...
        for item in items:
            ...
            if now >= deadline:
                ...
                audit_payload = {"ResourceId": resource_id, "Action": "PURGED", "Timestamp": now}
                s3.put_object(Bucket=BUCKET_NAME, Key=f"audit-logs/{resource_id}-{now}.json", Body=json.dumps(audit_payload))
